...
def upload():
    ...
    # persist metadata so the standalone workspace can poll readiness via /check_cache
    try:
        cache = Cache()
        cache.save(viewer_metadata)
        logger.info("Viewer metadata cached successfully")
    except Exception as e:
        logger.error("Failed to cache viewer metadata: %s", str(e), exc_info=True)
        return make_response({
            "error": "Viewer metadata caching failed",
