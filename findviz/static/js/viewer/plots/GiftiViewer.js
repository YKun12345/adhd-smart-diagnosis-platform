...
class GiftiViewer {
    ...
    plotGiftiDataUpdate(hemisphere=null, hoverTextOn=null) {
        ...
        if ((hemisphere == 'left') || (hemisphere == null && this.leftInput)) {
            let leftSurfaceUpdate = {
                ...
                hovertemplate: this.hoverTextOn ? '强度：%{intensity:.3f}<br>%{text}<extra></extra>' : null
            }
    ...
    plotHemisphere(
        ...
        const plotData = {
            ...
            hovertemplate: hoverTextOn ? '强度：%{intensity:.3f}<br>%{text}<extra></extra>' : null
        };
